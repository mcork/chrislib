 ...
 *  ChrisLib
 *
 *  Created by Christophe Soares on 09/09/14.
...
#else
					p << osc::BeginMessage( "/tuio/2Dcur" ) << "set" << temp.ID << temp.X << temp.Y << temp.dX << temp.dY << m << temp.width << temp.height << osc::EndMessage;
					//pressure
